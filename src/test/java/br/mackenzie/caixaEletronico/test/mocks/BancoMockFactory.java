...
public class BancoMockFactory {
	...
	public Banco getBancoNaoAprovaSaque(){	
		...
		expect(banco.sacar(anyObject(String.class), anyObject(Double.class))).andThrow(new Exception("Saque Não Aprovado"));
		replay(banco);		
		return banco;		
	}
	...
	public Banco getBancoAprovaSaque(){	
		Banco banco = createMock(Banco.class);
		expect(banco.sacar(anyObject(String.class), anyObject(Double.class))).andReturn(true);
		replay(banco);		
		return banco;		
	}
	...
	public Banco getBancoNaoAprovaDeposito(){	
		Banco banco = createMock(Banco.class);
		expect(banco.iniciarDeposito(anyObject(String.class), anyObject(Double.class))).andThrow(new Exception("Deposito Não Aprovado"));
		replay(banco);		
		return banco;		
	}
	...
	public Banco getBancoAprovaDeposito(){	
		Banco banco = createMock(Banco.class);
		expect(banco.iniciarDeposito(anyObject(String.class), anyObject(Double.class))).andReturn(true);
		replay(banco);		
		return banco;		
	}	
